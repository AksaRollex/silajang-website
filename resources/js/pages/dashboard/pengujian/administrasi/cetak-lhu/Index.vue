<template>
  <div class="card mb-10">
    ...
    <div class="card-body">
      <paginate ref="paginateBelum" id="table-belum" url="/administrasi/cetak-lhu" queryKey="table-belum"
        :columns="columnsBelum" :payload="{
          status: [6, 7],
          can_upload: 1,
          // start,
          // end,
  ...
  <div class="card">
    <div class="card-header align-items-center">
      ...
      <div class="d-flex align-items-center gap-5">
        ...
        <select2 placeholder="Pilih Tahun" class="form-select-solid mw-200px mw-md-100" name="tahunSudah"
          :options="tahuns" v-model="tahunSudah">
        </select2>
      </div>
    </div>
    <div class="card-body">
      <paginate ref="paginateSudah" id="table-sudah" url="/administrasi/cetak-lhu" queryKey="table-sudah"
        :columns="columnsSudah" :payload="{
          status: 8,
          // start,
          // end,
          tahun: tahunSudah,
  ...
  <form @submit.prevent="sendRevisi" class="modal fade" tabindex="-1" id="modal-revisi">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body fs-4">
          <textarea name="revisi" autocomplete="off" rows="3" class="form-control form-control-lg form-control-solid"
            placeholder="Keterangan Revisi" v-model="keteranganRevisi"></textarea>
        </div>
        <div class="modal-footer">
          <button class="btn btn-primary btn-sm">Revisi</button>
        </div>
      </div>
    </div>
  </form>
...
export default defineComponent({
    ...
    const columnsBelum = [
      ...
      column.accessor("uuid", {
        ...
        cell: (cell) => h('div', { class: 'd-flex gap-2 flex-wrap' }, [
          ...
          h('button', {
            class: 'btn btn-sm btn-light-warning', onClick: () => {
              selected.value = cell.getValue()
  ...
  methods: {
    ...
    sendRevisi() {
      ...
      axios.post(`/administrasi/${this.selected}/cetak-lhu/revisi`, { keterangan_revisi: this.keteranganRevisi })
        .then(() => {
