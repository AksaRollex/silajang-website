<template>
  ...
  <div v-if="!openDetail" class="card mb-10">
    ...
    <div class="card-body">
      <paginate ref="paginateKonfirmasi" id="table-konfirmasi" url="/verifikasi/koordinator-teknis"
        :columns="columnsBelum" queryKey="koordinator-teknis-0" :payload="{ status: 5, tahun: tahunKonfirmasi }">
      </paginate>
    </div>
  </div>
          ...
          h('button', {
            class: 'btn btn-sm btn btn-danger', onClick: () => {
              if (previewReport.value) {
                ...
                reportUrl.value = `/api/v1/report/${cell.getValue()}/rdps?token=${localStorage.getItem('auth_token')}`
                $('#modal-report').modal('show')
              } else {
                downloadReport(`/report/${cell.getValue()}/rdps`)
              }
            }
          }, [
            h('i', { class: 'la la-file-pdf fs-2' }),
            h('span', { class: 'd-none d-md-inline' }, 'RDPS')
          ]),
          h('button', {
