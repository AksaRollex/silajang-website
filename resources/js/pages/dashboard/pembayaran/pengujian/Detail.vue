<template>
  <div class="card mb-10">
    ...
    <div class="card-body pb-10">
      <template v-if="!formData.payment">
        ...
        <div class="alert alert-primary p-5 text-center">
          ...
          <div class="d-flex flex-column">
            <!--begin::Title-->
            <h4 v-if="formData.payment_type == 'va'" class="mb-1 text-dark">VA Pembayaran Belum Dibuat</h4>
            <h4 v-else-if="formData.payment_type == 'qris'" class="mb-1 text-dark">QRIS Belum Dibuat</h4>
            <!--end::Title-->

            <!--begin::Content-->
            <span v-if="formData.payment_type == 'va'">Silahkan klik Tombol Di Bawah untuk Membuat VA Pembayaran</span>
            <span v-else-if="formData.payment_type == 'qris'">Silahkan klik Tombol Di Bawah untuk Membuat QRIS</span>
            <!--end::Content-->
          </div>
          <!--end::Wrapper-->
        </div>
      ...
      <template v-else>
        <div class="row row-gap-8">
          ...
          moment(formData.payment.tanggal_bayar).format('DD-MM-YYYY') }}</strong></div>
          </div>
          ...
          <div class="col-12" v-else-if="formData.payment?.type == 'va'">
            <div class="bg-danger text-white py-1 rounded-1 text-center">VA Pembayaran telah kedaluwarsa</div>
          </div>
          <div class="col-12" v-else-if="formData.payment?.type == 'qris'">
            <div class="bg-danger text-white py-1 rounded-1 text-center">QRIS telah kedaluwarsa</div>
          </div>
          <div class="col-md-6">
            <div v-if="formData.payment?.type == 'va'"
              :class="`card ${formData.payment?.is_expired ? 'opacity-25' : ''}`"
            ...
            <div v-else-if="formData.payment?.type == 'qris'"
              :class="`card ${formData.payment?.is_expired ? 'opacity-25' : ''}`"
              :style="`cursor: ${formData.payment?.is_expired ? 'not-allowed' : 'default'}`">
              <div class="card-header align-items-center min-h-50px">
              ...
              <div class="card-body p-4">
                <div class="d-flex bg-light rounded-2 p-4 gap-8 flex-column align-items-center">
                  ...
                  <button class="btn text-success p-0" :disabled="formData.payment?.is_expired" type="button"
                    @click="downloadQris()">Unduh QRIS</button>
                </div>
              </div>
            </div>
          </div>
    ...
    <div v-if="formData.payment?.status !== 'success'" class="card-footer d-flex justify-content-end gap-4">
      <template v-if="formData.status > 1 && formData.payment?.status == 'pending' && !formData.payment?.is_expired">
        <button class="btn btn-sm btn-light-danger" type="button"
          @click="cancelPembayaran(`/pembayaran/pengujian/${this.selected}/cancel`, 'POST')">
          Batalkan
        </button>
        <button v-if="formData.payment_type == 'va'" class="btn btn-sm btn-light-primary" type="button"
          @click="checkPembayaran()">
          <i class="la la-refresh"></i>
          Cek Status
        </button>
      </template>
      <template v-if="(formData.status > 1 && !formData.payment) || formData.payment?.is_expired">
        <button v-if="formData.payment_type == 'va'" type="button" class="btn btn-sm btn-primary" @click="generate()">
          Buat VA Pembayaran
        </button>
        <button v-else-if="formData.payment_type == 'qris'" type="button" class="btn btn-sm btn-primary"
          @click="generateQris()">
          Buat QRIS
        </button>
      </template>
...
export default defineComponent({
  ...
  methods: {
      ...
      axios.get(`/pembayaran/pengujian/${this.selected}`)
        .then(res => {
          ...
          if (this.formData.payment) {
            ...
            if (this.formData.payment.type == 'qris') {
              QRCode.toDataURL(this.formData.payment.qris_value)
                .then(url => {
                  this.qris = url
                })
                .catch(err => {
    ...
    generateQris() {
      ...
      axios.post(`/pembayaran/pengujian/${this.selected}/qris`)
        .then(res => {
          toast.success(res.data.message)
          this.getEdit()
        })
    ...
    checkPembayaran() {
      ...
      axios.post(`/pembayaran/pengujian/${this.selected}/check`)
        .then(res => {
          toast.success(res.data.message)
          this.getEdit()
        })
