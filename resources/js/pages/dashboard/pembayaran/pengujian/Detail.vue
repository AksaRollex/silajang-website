    ...
    <div class="card mb-10">
        ...
        <div v-if="formData.payment?.status !== 'success'" class="card-footer d-flex justify-content-end gap-4">
            <template v-if="formData.status > 1 && formData.payment?.status == 'pending' && !formData.payment?.is_expired">
                ...
                <button class="btn btn-sm btn-light-primary" type="button" @click="checkPembayaran()">
                    <i class="la la-refresh"></i>
                    Cek Status
                </button>
            </template>
            <button type="button" class="btn btn-sm btn-primary" @click="generate()"
                v-if="(formData.status > 1 && !formData.payment) || formData.payment?.is_expired">Buat
                VA Pembayaran</button>
        </div>
    </div>
</template>
