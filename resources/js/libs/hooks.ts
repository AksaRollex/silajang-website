...
export const useDownloadWord = (callback?: ICallback, swalMixin?: any) => {
    ...
    return {
        'download': (url: string | Array<string>, method: string = 'GET', body: any = {}) => mySwal.fire({
            title: 'Apakah Anda Yakin?',
            ...
            preConfirm: function (login) {
                if (typeof url === 'string') {
                    return axios(url, {
                        ...
                        window.respon = JSON.parse(String.fromCharCode.apply(null, new Uint8Array(error.response.data)))
                    });
                } else {
                    let promises: Array<any> = [];
                    for (let i = 0; i < url.length; i++) {
                        promises.push(axios(url[i], {
                            method: method,
                            responseType: 'arraybuffer',
                            data: body
                        }).then((res) => {
            ...
            if (result.isConfirmed) {
                if (window.respon.status) {
                    mySwal.fire('Berhasil!', 'File Berhasil Di Download.', 'success')
                } else {
                    mySwal.fire('Error!', window.respon.message, 'error')
                }
            }
        })
    }
...
export const useSwalConfirm = (options?: any, callback?: ICallback, swalMixin?: any) => {
    const mySwal = Swal.mixin(swalMixin || {
        customClass: {
    ...
    return {
        'confirm': (url: string, method: string = 'GET', body: any = {}) => mySwal.fire({
            title: 'Apakah Anda Yakin?',
            icon: 'warning',
            showCancelButton: true,
            showLoaderOnConfirm: true,
            ...
            preConfirm: function (login) {
                return axios(url, {
                    ...
                    window.respon = { status: true, message: res.data }
                }).catch((error) => {
                    onError && onError();
            ...
            if (result.isConfirmed) {
                if (window.respon?.status) {
                    mySwal.fire('Berhasil!', window.respon?.message, 'success')
                } else {
                    mySwal.fire('Error!', window.respon?.message, 'error')
                }
            }
