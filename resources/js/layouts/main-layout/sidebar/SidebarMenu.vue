<template>
  ...
  <div class="app-sidebar-menu overflow-hidden flex-column-fluid">
    ...
    <div id="kt_app_sidebar_menu_wrapper" class="app-sidebar-wrapper hover-scroll-overlay-y my-5" data-kt-scroll="true"
      ...
      <!--begin::Menu-->
      <div v-if="user?.permission && (user?.detail || user?.golongan_id == 2 || user?.role?.name == 'admin')"
        id="#kt_app_sidebar_menu" class="menu menu-column menu-rounded menu-sub-indention px-3" data-kt-menu="true">
        <template v-for="(item, i) in MainMenuConfig" :key="i">
          ...
          <template v-for="(menuItem, j) in item.pages" :key="j">
            <template v-if="menuItem.heading && checkPermission(menuItem.name)">
              <div class="menu-item">
                <router-link v-if="menuItem.route" class="menu-link" exact-active-class="active" :to="menuItem.route">
              ...
              <div :class="{ show: hasActiveChildren(menuItem.route) }" class="menu-sub menu-sub-accordion">
                <template v-for="(item2, k) in menuItem.sub" :key="k">
                  <div v-if="item2.heading && checkPermission(item2.name)" class="menu-item">
                    <router-link v-if="item2.route" class="menu-link" exact-active-class="active" :to="item2.route">
                    ...
                    <div :class="{ show: hasActiveChildren(item2.route) }" class="menu-sub menu-sub-accordion">
                      <template v-for="(item3, k) in item2.sub" :key="k">
                        <div v-if="item3.heading && checkPermission(item3.name)" class="menu-item">
                          <router-link v-if="item3.route" class="menu-link" exact-active-class="active" :to="item3.route">
                            <span class="menu-bullet">
                              <span class="bullet bullet-dot"></span>
                            </span>
                            <span class="menu-title">{{
