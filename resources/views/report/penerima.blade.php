...
<head>
    ...
    <title>Tanda Terima {{ $data->no_formulir }}</title>
</head>
...
<body>
    ...
    <table class="main" cellpadding="8" cellspacing="0">
        ...
        <tr>
            ...
            <td colspan="15">{{ @$data->permohonan->user->detail->telepon ?? $data->permohonan->user->phone }} /
                {{ @$data->permohonan->user->detail->email ?? $data->permohonan->user->email }}</td>
        </tr>
        <tr>
            <td colspan="3">
                <div>Tanggal Sampel Diterima :</div>
                <div>
            ...
            <td>
                <div>Jam Datang :</div>
                <div>{{ Illuminate\Support\Carbon::parse($data->tanggal_diterima)->format('H:i') }}</div>
            </td>
            <td colspan="2">
                <div>Tanggal Perkiraan Selesai Pengujian :</div>
                <div>
                    {{ App\Helpers\AppHelper::tanggal_indo(Illuminate\Support\Carbon::parse($data->tanggal_selesai)->format('Y-m-d')) }}
                </div>
            </td>
            <td colspan="5">
                <div>Pelanggan Pengirim Sampel :</div>
                <div>{{ $data->permohonan->user->nama }}</div>
            </td>
            <td colspan="7" rowspan="2">
                <div style="margin-bottom: 1rem">Kondisi Sampel Saat Diterima :</div>
                <div>
                    <span class="checkbox check">
                        @if ($data->kondisi_sampel == 1)
                            V
                        @endif
                    </span>
        ...
        <tr>
            <td colspan="6" rowspan="2">
                <div>Acuan Perundangan yang Digunakan :</div>
                <div>{{ @$data->peraturan->nama }} {{ @$data->peraturan->nomor }}</div>
            </td>
            <td colspan="5">
                <div>Nama Petugas Pengambil Sampel :</div>
                <div>{{ @$data->pengambil->nama }}</div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div style="margin-bottom: 1rem">Kesimpulan Sampel :</div>
                <div>
                    <span class="checkbox check">
                        @if ($data->kesimpulan_sampel == 1)
                            V
                        @endif
                    </span>
                    <span style="margin-right: 2rem">Diterima</span>

                    <span class="checkbox check">
                        @if ($data->kesimpulan_sampel == 2)
                            V
                        @endif
                    </span>
                    <span>Ditolak</span>
            ...
            <td colspan="7">
                <div style="margin-bottom: 1rem">Interpretasi Pengujian :</div>
                <div>
                    <span class="checkbox check">
