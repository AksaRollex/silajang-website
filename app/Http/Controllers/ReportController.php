...
class ReportController extends Controller {
  public function index(Request $request) {
    if (request()->wantsJson()) {
      ...
      $data = TitikPermohonan::with(['permohonan.user.detail'])->where(function ($q) use ($request) {
        ...
        // ->whereBetween('tanggal_diterima', [Carbon::parse($request->start)->startOfDay(), Carbon::parse($request->end)->endOfDay()])
        ->whereYear('tanggal_diterima', $request->tahun)->whereMonth('tanggal_diterima', $request->bulan)
        ->orderBy('kode', 'DESC')
  ...
  public function reportSKRD(Request $request, $uuid, $tte = false, $base64 = false) {
    if ($request->multi_payment == 1) {
      $data = Payment::with(['multiPayments.titikPermohonan' => function ($q) {
        $q->with(['permohonan.user.detail', 'parameters']);
    ...
    if ($request->multi_payment == 1) {
      $parametersUji = collect([]);
      foreach ($data->multiPayments as $payment) {
        $parametersUji = $parametersUji->merge($payment->titikPermohonan->parameters()->where('is_dapat_diuji', 1)->get());
      }

      $parametersUji = $parametersUji->unique('id');
      $permohonan = $data->multiPayments->first()->titikPermohonan->permohonan;
    } else {
      $parametersUji = $data->parameters()->where('is_dapat_diuji', 1)->get();
    ...
    $pdf = Pdf::loadview('report.skrd', compact('data', 'tte', 'qrCode', 'parametersUji', 'ttdBayar', 'ttdUPT', 'setting', 'permohonan'));
    $pdf->setPaper('A3');
    $pdf->output();
  ...
  public function reportKwitansi(Request $request, $uuid, $tte = false, $base64 = false) {
    if ($request->multi_payment == 1) {
      $data = Payment::with(['multiPayments.titikPermohonan' => function ($q) {
        $q->with(['permohonan.user.detail']);
      }])->where('uuid', $uuid)->first();
    } else {
      $data = TitikPermohonan::with(['permohonan' => function ($q) {
        $q->with(['user.detail']);
      }, 'payment'])->where('uuid', $uuid)->first();
    }
    ...
    $pdf = Pdf::loadview('report.kwitansi', compact('data', 'tte', 'qrCode', 'ttd', 'setting', 'permohonan'));
  ...
  public function reportPembayaranPengujian(Request $request) {
    $data = TitikPermohonan::has('payment')->with(['permohonan' => function ($q) {
      $q->with(['user.detail']);
    }, 'payment'])->where('payment_type', $request->type)->whereYear('created_at', $request->tahun)->whereMonth('created_at', $request->bulan)->orderBy('created_at', 'desc')->get();
  ...
  public function reportPembayaranMultiPayment(Request $request) {
    $data = Payment::with(['multiPayments.titikPermohonan.permohonan.user.detail'])->has('multiPayments')->where('type', $request->type)->whereYear('created_at', $request->tahun)->whereMonth('created_at', $request->bulan)->orderBy('created_at', 'desc')->get();

    $data = $data->map(function ($a) {
      $a->nama = $a->multiPayments->first()->titikPermohonan->permohonan->user->nama;
      return $a;
    });
    $pdf = Pdf::loadview('report.pembayaran-multi-payment', compact('data'));
    $pdf->setPaper('A3');
    $pdf->output();

    return $pdf->stream('Pembayaran-Multi-Payment-' . Carbon::now()->format('d-m-Y') . '.pdf');
  }

  public function reportPembayaranNonPengujian(Request $request) {
    $data = Payment::where('titik_permohonan_id', null)->where('payment_type', $request->type)->whereYear('created_at', $request->tahun)->orderBy('created_at', 'desc')->get();
