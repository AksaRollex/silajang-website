...
class AppHelper
{
	static function cetakMiringParamter(Parameter $parameter)
	{
		// ID dari parameter yang akan dicetak miring
		$parameters_miring = Parameter::where('cetak_miring', 1)->get()->pluck('id')->toArray();
		if (in_array($parameter->id, $parameters_miring)) {
			return true;
		}
		return false;
	}
